...
@section('content')
  ...
  <form action="{{ route('articles.update', [$lang, $article->id]) }}" method="post" enctype="multipart/form-data">
    <input type="hidden" name="_method" value="PUT">
    {!! csrf_field() !!}

    <div class="form-group">
      <label for="title">Название</label>
      <input type="text" class="form-control" id="title" name="title" minlength="2" maxlength="80" value="{{ (old('title')) ? old('title') : $article->title }}" required>
    </div>
    <div class="form-group">
      <label for="slug">URI</label>
      <input type="text" class="form-control" id="slug" name="slug" minlength="2" maxlength="80" value="{{ (old('slug')) ? old('slug') : $article->slug }}">
    </div>
    <div class="form-group">
      <label for="page_id">Категории</label>
      <select id="page_id" name="page_id" class="form-control">
        <option value="NULL"></option>
        <?php $traverse = function ($nodes, $prefix = null) use (&$traverse, $article) { ?>
          <?php foreach ($nodes as $node) : ?>
            <?php if ($node->id == $article->page_id) : ?>
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="headline" name="headline" minlength="2" maxlength="500" value="{{ (old('headline')) ? old('headline') : $article->headline }}">
    </div>
    <div class="form-group">
      <label for="video">Код видео</label>
      <input type="text" class="form-control" id="video" name="video" minlength="2" maxlength="500" value="{{ (old('video')) ? old('video') : $article->video }}">
    </div>
    <div class="form-group">
      <label for="sort_id">Номер</label>
      <input type="text" class="form-control" id="sort_id" name="sort_id" maxlength="5" value="{{ (old('sort_id')) ? old('sort_id') : $article->sort_id }}">
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="meta_title" name="meta_title" maxlength="255" value="{{ (old('meta_title')) ? old('meta_title') : $article->meta_title }}">
    </div>
    <div class="form-group">
      <label for="meta_description">Мета описание</label>
      <input type="text" class="form-control" id="meta_description" name="meta_description" maxlength="255" value="{{ (old('meta_description')) ? old('meta_description') : $article->meta_description }}">
    </div>
    <div class="form-group">
      <label for="content">Контент</label>
      <textarea class="form-control" name="content" rows="5">{{ (old('content')) ? old('content') : $article->content }}</textarea>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
      ...
      <label>
        <input type="checkbox" id="status" name="status" @if ($article->status == 1) checked @endif> Активен
      </label>
    </div>
    <div class="form-group">
